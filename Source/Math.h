...
public:
	...
	explicit Matrix2(float inMat[2][2])
	{
		memcpy(mat, inMat, 4 * sizeof(float));
	}

	// Return contiguous float pointer (useful para OpenGL, etc.)
	const float *GetAsFloatPtr() const
	{
	...
	{
		Matrix2 r;
		r.mat[0][0] = lhs.mat[0][0] * rhs.mat[0][0] + lhs.mat[0][1] * rhs.mat[1][0];
		r.mat[0][1] = lhs.mat[0][0] * rhs.mat[0][1] + lhs.mat[0][1] * rhs.mat[1][1];

		r.mat[1][0] = lhs.mat[1][0] * rhs.mat[0][0] + lhs.mat[1][1] * rhs.mat[1][0];
		r.mat[1][1] = lhs.mat[1][0] * rhs.mat[0][1] + lhs.mat[1][1] * rhs.mat[1][1];
		return r;
	}
	...
	// Escala não-uniforme
	static Matrix2 CreateScale(float xScale, float yScale)
	{
		float tmp[2][2] = {{xScale, 0.0f}, {0.0f, yScale}};
		return Matrix2(tmp);
	}

	static Matrix2 CreateScale(const Vector2 &s) { return CreateScale(s.x, s.y); }
	static Matrix2 CreateScale(float uniform) { return CreateScale(uniform, uniform); }

	// Rotação em radianos (mesma convenção usada no seu Matrix3)
...
{
	return Vector2(
		v.x * m.mat[0][0] + v.y * m.mat[1][0],
		v.x * m.mat[0][1] + v.y * m.mat[1][1]
	);
}
